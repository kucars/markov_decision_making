...
agent1Loc = {'a','b', 'c','d','e','f','g'};
agent2Loc = {'a','b', 'c','d','e','f','g'};
...
% a,b,c,d,e,f,g {right, left, up, down, stop, clear/extract}
%a=1,b=2,c=3,d=4,e=5,f=6. g=7
network =[[1,2,1,1,1,1];[1,2,3,2,2,2];[3,6,4,2,3,3];[5,4,4,3,4,4];[5,4,5,5,5,5];[3,6,6,7,6,6];[7,7,6,7,7,7]];

format long; 
% Multi-Agent Human Robot Collaboration
outputFile = 'MAHRC_4x4_v2_1.dpomdp';  
...
%----------------- penality for human go to danger------------------------- 
	  ...
	  for x=1:length(agent1Loc)
	    for v=1:length(victimLoc)
	      %if(a2<3 )%remove && z~=2
                fprintf(fid,'\nR: %s right : %s_f_%s_%s : * :  * : -69', agent1Actions{a1}, agent1Loc{x},victimLoc{v},dangerlocNode);
                fprintf(fid,'\nR: %s up : %s_b_%s_%s : * :  * : -100', agent1Actions{a1}, agent1Loc{x},victimLoc{v},dangerlocNode);
		fprintf(fid,'\nR: %s down : %s_d_%s_%s : * :  * : -100', agent1Actions{a1}, agent1Loc{x},victimLoc{v},dangerlocNode);
