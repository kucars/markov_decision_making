...
outputFile = 'MAHRC_1x3_v1_3.dpomdp';  
	...
	for a3=1:length(agent3Actions)
	  ...
	  for k=1:length(dangerLoc)
		...
		for o3=1:length(agent3Observation) 
		    ...
		    fprintf(fid,'\nO: %s %s : %s_%s_%s_%s :  %s %s :%f',agent1Actions{a1},agent3Actions{a3},endStateX,endStateZ,victimLoc{w},dangerLoc{k},agent1Observation{o1},agent3Observation{o3},uniprob1);
		end 
	      end 
 ...
 for a1=1:length(agent1Actions)
      for x=1:length(nodesX)
          for xxx=1:length(victimLoc)
                fprintf(fid,'\nR: %s stop : %s_b_%s_b : * :  * : -90', agent1Actions{a1}, nodesX{x}, victimLoc{xxx});
                fprintf(fid,'\nR: %s extract_victim : %s_b_%s_b : * :  * : -90', agent1Actions{a1}, nodesX{x}, victimLoc{xxx});
	...
	for x=1:length(nodesX)
		for xxx=1:length(nodesZ)
			...
			if ( strcmp(nodesZ{xxx},'a') ||  strcmp(nodesZ{xxx},'c'))
				  ...
				  fprintf(fid,'\nR: %s stop : %s_%s_a_b : * : * : 70', agent1Actions{a1},nodesX{x}, nodesZ{xxx});
				  fprintf(fid,'\nR: %s stop : %s_%s_n_b : * : * : 70', agent1Actions{a1},nodesX{x}, nodesZ{xxx});
			      end
			end
		     end
		end 
...
for a3=1:length(agent3Actions)
  for xxx=1:length(nodesZ)
    for xxxx=1:length(victimLoc)                
      fprintf(fid,'\nR:  clear_danger %s : b_%s_%s_b :  * : * : 70',agent3Actions{a3}, nodesZ{xxx}, victimLoc{xxxx});
    end
  end 
end
