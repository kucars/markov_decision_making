...
outputFile = 'MAHRC_1x3_v1_6.dpomdp';  
        ...
        for k=1:length(dangerLoc)
		  ...
		  %------x agent --------------------
                    if(network(x,a1)~=0)
                        ...
                        if(strcmp(endStateX(1), 'b') && k==1)
			  obsX(1)=2;
                        elseif (strcmp( endStateX(1), 'a') && w==1)
			  obsX(1)=1;
                        else
			  obsX(1)=3;
                        end
                       ...
                       if(strcmp(endStateX(2), 'b') && k==1)
			  obsX(2)=2;
                        elseif (strcmp( endStateX(2), 'a') && w==1)
			  obsX(2)=1;
                        else
			  obsX(2)=3;
                        end
			  ...
			  obsX(2)=3;
                    ...
                    %-----z agent---------------------
                     if(network(z,a3)~=0)
                        ...
                        if(strcmp(endStateZ(1), 'b') && k==1)
			  obsZ(1)=2;
                        elseif (strcmp( endStateZ(1), 'a') && w==1)
			  obsZ(1)=1;
                        else
			  obsZ(1)=3;
                        end
                        ...
                        if(strcmp(endStateZ(2), 'b')  && k==1)
                          obsZ(2)=2;
                        elseif (strcmp(endStateZ(2), 'a')  && w==1)
                          obsZ(2)=1;
                        else
                          obsZ(2)=3;
                        end
			  ...
			  obsZ(1)=3;
                  ...
                  if(o1==obsX(1))
                      prob01(1)=o_uncertainty;
                  else
                      prob01(1)=(1-o_uncertainty)/2;
                  end

                  if(o1==obsX(2))
                      prob01(2)=o_uncertainty;
                  else
                      prob01(2)=(1-o_uncertainty)/2;
                  end
                 
                 if(o3==obsZ(1))
			probO3(1)=o_uncertainty;
                  else
                      probO3(1)=(1-o_uncertainty)/2;
                  end

                  if(o3==obsZ(2))
		      probO3(2)=o_uncertainty;
                  else
                      probO3(2)=(1-o_uncertainty)/2;
                  end
                  
                  uniprob1=prob01(1)*probO3(1);
                  uniprob2=prob01(2)*probO3(2);;
                  ...
                  fprintf(fid,'\nO: %s %s : %s_%s_%s_%s :  %s %s :%f',agent1Actions{a1},agent3Actions{a3},endStateX(2),endStateZ(2),victimLoc{w},dangerLoc{k},agent1Observation{o1},agent3Observation{o3},uniprob2);
...
%  		end 
%  	end 
% -----------------Penality for human going toward the danger----------------------------------
for a1=1:length(agent1Actions)
  for x=1:length(nodesX)
    for v=1:length(victimLoc)   
      fprintf(fid,'\nR:  %s right : %s_a_%s_b : * : * : -100', agent1Actions{a1},nodesX{x},victimLoc{v});
      fprintf(fid,'\nR:  %s left : %s_c_%s_b : * : * : -100', agent1Actions{a1},nodesX{x},victimLoc{v});
    end
   end
end 
